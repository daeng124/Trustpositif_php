...
$seq = $_GET['seq'];
$query = mysqli_query($koneksi, "SELECT * FROM tb_domains WHERE seq=$seq");
$domain = mysqli_fetch_array($query);
...
if ($response_html == $tidak_ketemu) {
    header('Location: index.php?page=cek-ssl');
} else {
    $re = '/expires\s.*[(]/m'; // patern untuk mencari kata di regex online 101 
    preg_match_all($re, $response_html, $matches, PREG_SET_ORDER, 0);
    $hasil2 = str_replace('expires', '', $matches[0][0]);
    $hasil2 = str_replace('(', '', $hasil2);
    $hasil2 = trim($hasil2);
    $hasil2 = strtotime($hasil2);
    $tanggal_expired2 = date('Y-m-d', $hasil2);
    // echo $tanggal_expired2 . "<br>";

    $query3 = "UPDATE `domain_trustpositif`.`tb_domains` SET `ssl_exp`='$tanggal_expired2' WHERE  `seq`= $seq";
